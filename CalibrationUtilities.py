...
def stack_images(img_array, scale, labels=None):
    if labels is None:
        labels = []
    rows = len(img_array)
        ...
        for d in range(0, rows):
            for c in range(0, cols):
                cv2.rectangle(ver, (c * each_img_width, each_img_height * d),
                              (c * each_img_width + len(labels[d]) * 13 + 27, 30 + each_img_height * d),
                              (255, 255, 255),
                              cv2.FILLED)
                cv2.putText(ver, str(labels[d]), (each_img_width * c + 10, each_img_height * d + 20),
